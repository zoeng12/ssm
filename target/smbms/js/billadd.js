		...
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				if(data.billCode == "exist"){//账号已存在，错误提示
					validateTip(billCode.next(),{"color":"red"},imgNo+ " 该商品编码已存在",false);
				}else if (data.billCode == "billCodeIsNull"){
					validateTip(billCode.next(),{"color":"red"},imgNo+ " 商品编码不能为空",false);
				} else{//账号可用，正确提示
					validateTip(billCode.next(),{"color":"green"},imgYes+" 该商品编码可以使用",true);
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
